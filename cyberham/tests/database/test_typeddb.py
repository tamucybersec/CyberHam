...
class TestTypedDB(BackendPatcher):
    ...
    def test_create_item(self):
        ...
        after = usersdb.get([unregistered_user[pk_name]])
    ...
    def test_get_item(self):
        item = usersdb.get([valid_user[pk_name]])
        assert item and valid_user == item

    def test_get_non_existent_item(self):
        item = usersdb.get([unregistered_user[pk_name]])
        assert item is None

    def test_update_item_using_pk(self):
        # create
        item = usersdb.update(_test_update_func, pk_values=[unregistered_user[pk_name]])
    ...
    def test_delete_item(self):
        old_item = usersdb.delete([valid_user[pk_name]])
        assert old_item == valid_user

    def test_delete_non_existent_item(self):
        old_item = usersdb.delete([unregistered_user[pk_name]])
