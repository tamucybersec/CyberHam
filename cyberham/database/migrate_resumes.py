...
def migrate_resumes():
    ...
    for user_id in os.listdir(
        data_path / "resumes"
    ):  # files get written to the user_id of their uploader
        ...
        upload_date = (
            datetime.fromtimestamp(os.path.getmtime(resume_path), tz=UTC)
            .isoformat()
            .replace("+00:00", "Z")
        )
    ...
    if (
        len(has_errors) == 0
    ):  # so it doesn't delete the possibly existing data for those resumes until that's dealt with
