...
def setup_commands(bot: Bot):
    ...
    async def generate_RSVP_form(
        interaction: discord.Interaction, code: str = "", date: str = ""
    ):
        ...
        if date == "":
            ...
            await interaction.response.send_message(
                "Invalid date! Please return a valid date in the format mm/dd/yy or mm/dd/yyyy.",
                ephemeral=True,
            )
            return
        ...
        channel = cast(
            discord.TextChannel,
            await bot.fetch_channel(activity_group_channels[channel_id]),
        )
        ...
        await interaction.response.send_message(
            "RSVP question sent: " + question, ephemeral=True
        )
    ...
    async def count_rsvp(interaction: discord.Interaction, code: str = ""):
        await interaction.response.send_message(
            backend_events.count_rsvp_event(code), ephemeral=True
        )
