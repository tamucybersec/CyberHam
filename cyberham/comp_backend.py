...
client = Elasticsearch(
    es_conf.endpoints, api_key=(es_conf.id, es_conf.api_key), verify_certs=False
)
...
def init_db():
    ...
    if not client.indices.exists(index=f"events-{es_conf.index_postfix}"):
        ...
        client.indices.create(
            index=f"events-{es_conf.index_postfix}", mappings=mappings
        )
    ...
    if not client.indices.exists(index=f"events-attendance-{es_conf.index_postfix}"):
        ...
        client.indices.create(
            index=f"events-attendance-{es_conf.index_postfix}", mappings=mappings
        )
...
def create_event(name: str, points: int, date: str, resources: str, user_id: int):
    ...
    while keep_generating:
        ...
        if c.fetchone():  # returns tuple of one if exists otherwise none
            ...
            client.search(
                index=f"events-{es_conf.index_postfix}", query={"match": {"code": code}}
            )["hits"]["total"]["value"]
            == 1
        ):
            keep_generating = True
    ...
    new_id = client.search(
        index=f"events-{es_conf.index_postfix}", query={"match": {"user_id": user_id}}
    )
...
def attend_event(code: str, user_id: int, user_name: str):
    ...
    if (
        client.search(index=f"events-{es_conf.index_postfix}", body=code_query)["hits"][
            "total"
        ]["value"]
        == 0
    ):
    ...
    if current_day > event_day:
        ...
        client.update(
            ...
            doc={
                ...
                + client.search(
                    index=f"events-{es_conf.index_postfix}", body=code_query
                )["hits"]["hits"][0]["_source"]["points"],
            },
        )
...
def register(
    ...
    user = client.search(
        index=f"users-{es_conf.index_postfix}", query={"match": {"user_id": user_id}}
    )
    try:
...
def register_email(user_id, email, guild_id):
    query = {"query": {"match": {"user_id": user_id}}, "_source": ["email"]}
    ...
    if temp["hits"]["total"]["value"] == 0 and user_id not in pending_emails:
        ...
        if flagged["hits"]["total"]["value"] == 0:
            client.index(
                index=f"flagged-{es_conf.index_postfix}",
                body={"user_id": user_id, "offences": 1},
            )
        else:
            client.update(
                index=f"flagged-{es_conf.index_postfix}",
    ...
    query2 = {"query": {"match": {"email": email}}, "_source": ["user_id"]}
...
def verify_email(code: int, user_id: int):
    if user_id in pending_emails:
        ...
        if pend_email.code == code:
            my_id = client.search(
                index=f"users-{es_conf.index_postfix}", client={"user_id": user_id}
            )
...
def profile(user_id: int):
    document = client.search(
        index=f"users-{es_conf.index_postfix}", query={"user_id": user_id}
    )
...
def find_event(code: str = "", name: str = ""):
    ...
    if name == "" and code == "":
        ...
        data = client.search(
            index=f"events-{es_conf.index_postfix}", query={"name": name}
        )
    elif name == "":
        data = client.search(
            index=f"events-{es_conf.index_postfix}", query={"code": code}
        )
    else:
        data = client.search(
            index=f"events-{es_conf.index_postfix}", query={"name": name, "code": code}
        )
...
def event_list():
    data = client.search(
        index=f"events-{es_conf.index_postfix}", query={"match_all": {}}
    )
